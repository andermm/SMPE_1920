...
- This is the LabBook for the *Scientic Methodology and Performance
  Evaluation for Experimental Computer* class, in which the
  experimental informations are described.

* Experimental Project

** Objective
   Evaluate both InfiniBand and Gigabit Ethernet interconnections,
   using the same physical cluster of servers, executing the NAS
   ...
   using *Gigabit Ethernet (ETH)*, *Infiniband (IB)* and *IP-over-IB
   (IPoIB)* with 128 processes over 4 nodes (32 processes per node) in
...
** Experiments Execution
   ...
   [[./SMPE_1920/BATCH/nas.batch]] which executes the system
   information collector script ([[./SMPE_1920/BATCH/nas.batch]]) in the
   four nodes, install the dependencies required to compile the NPB
   benchmarks, download and compile it, read the experimental project, prepare the
   MPI line command for the execution, execute the experiments, and
